...
| Model accuracy degrades over time as studio context changes (concept drift) | 3 | 4 | **12 (Red)** | ✓ Root cause: As the studio's member base, class schedule, pricing, or demographics evolve, the relationship between visit behaviour and churn changes. A model trained once will gradually flag the wrong people, losing coach trust and collapsing adoption. ✓ Retraining Schedule: Model retrained every 6 months per studio on accumulated real outcome data. Minimum accuracy ≥ 80% on held-out test set before new model goes live. ✓ Drift Trigger Metric: The Stage 1 churn rate ratio (flagged members cancelling at ≥ 2× rate of unflagged) is tracked monthly. If ratio drops below 1.5 on two consecutive months, retraining is triggered ahead of schedule. ✓ Fallback: If retrained model accuracy < 80%, system reverts to the baseline model (gym_churn_us.csv, 92.5% accuracy) until a valid retrained model is ready. ✓ Support requirement: Monthly drift monitoring is a standard activity included in the retainer. This is not automated — it requires consultant review of outcome data per studio. |
